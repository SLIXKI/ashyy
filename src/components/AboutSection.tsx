import FadeIn from './FadeIn'
import AnimatedText from './AnimatedText'
import ContactButton from './ContactButton'

export default function AboutSection() {
  return (
    <section id="about" className="min-h-screen px-5 sm:px-8 md:px-10 py-20 bg-[#0C0C0C] relative overflow-hidden flex flex-col items-center justify-center gap-10 sm:gap-14 md:gap-16">
      {/* 3D decorative images */}
      <FadeIn delay={0.1} x={-80} y={0} duration={0.9} className="absolute top-[4%] left-[1%] sm:left-[2%] md:left-[4%] w-[120px] sm:w-[160px] md:w-[210px] pointer-events-none">
        <img src="https://shrug-person-78902957.figma.site/_components/v2/ebb2b8f25d8e24d5f0a5ca8af4c950de81aa2fd7/moon_icon.11395d36.png" alt="" className="w-full h-auto" />
      </FadeIn>
      <FadeIn delay={0.25} x={-80} y={0} duration={0.9} className="absolute bottom-[8%] left-[3%] sm:left-[6%] md:left-[10%] w-[100px] sm:w-[140px] md:w-[180px] pointer-events-none">
        <img src="https://shrug-person-78902957.figma.site/_components/v2/ebb2b8f25d8e24d5f0a5ca8af4c950de81aa2fd7/p59_1.4659672e.png" alt="" className="w-full h-auto" />
      </FadeIn>
      <FadeIn delay={0.15} x={80} y={0} duration={0.9} className="absolute top-[4%] right-[1%] sm:right-[2%] md:right-[4%] w-[120px] sm:w-[160px] md:w-[210px] pointer-events-none">
        <img src="https://shrug-person-78902957.figma.site/_components/v2/ebb2b8f25d8e24d5f0a5ca8af4c950de81aa2fd7/lego_icon-1.703bb594.png" alt="" className="w-full h-auto" />
      </FadeIn>
      <FadeIn delay={0.3} x={80} y={0} duration={0.9} className="absolute bottom-[8%] right-[3%] sm:right-[6%] md:right-[10%] w-[130px] sm:w-[170px] md:w-[220px] pointer-events-none">
        <img src="https://shrug-person-78902957.figma.site/_components/v2/ebb2b8f25d8e24d5f0a5ca8af4c950de81aa2fd7/Group_134-1.2e04f3ce.png" alt="" className="w-full h-auto" />
      </FadeIn>

      <FadeIn delay={0} y={40}>
        <h2 className="hero-heading font-black uppercase leading-none tracking-tight text-center" style={{ fontSize: 'clamp(3rem, 12vw, 160px)' }}>
          About me
        </h2>
      </FadeIn>

      <div className="flex flex-col items-center gap-16 sm:gap-20 md:gap-24 max-w-[560px] w-full">
        <AnimatedText
          text="With more than two years of experience in reverse engineering, i focus on payment gateways, BIN tools, and integrity bypass, i truly enjoy building tools that make life smoother for real people. ASHEO 1.6.1 is 5.9KB manifest, 2865 lines handcrafted UI, hashed and signed, Telegram only, no face, just work. Let's build something incredible together!"
          className="text-[#D7E2EA] font-medium leading-relaxed text-center"
        />
        <FadeIn delay={0.2} y={20}>
          <ContactButton />
        </FadeIn>
      </div>

      <div className="mt-10 grid grid-cols-2 md:grid-cols-4 gap-3 w-full max-w-4xl">
        {[
          { k: 'No Face', v: 'Ghost • Anonymous • Real' },
          { k: 'Solo • 2 Years', v: 'No team • No funding' },
          { k: '0.8s Avg', v: '116+ gateways • Real' },
          { k: '14+ Proofs', v: '100% real • 0 fake' },
        ].map((s, i) => (
          <div key={i} className="bg-white/[0.04] border border-white/10 rounded-2xl p-4">
            <div className="font-mono text-[10px] tracking-widest uppercase text-white/40">{s.k}</div>
            <div className="font-kanit font-semibold text-white mt-1 text-sm">{s.v}</div>
          </div>
        ))}
      </div>
    </section>
  )
}
